...
| Household recovery `salt` (16 B) — **A** | NVS `HH_REC_SALT` | KDF salt | household-wide, public | Yes (backup header) | Yes | No | No | Salt is not secret; carried in the backup header, and reported by `GET /household` and `POST /recovery/export` so a client can derive the command key. |
