...
| Household recovery `secret` (32 B) — **C** | NVS `HH_RECOVERY_SECRET` | Encrypt backups / authenticate MQTT commands | household-wide, secret | No (it *is* the backup key source) | Yes — user re-supplies the offline copy | No (regenerating loses access to existing backups) | No | One-time export; decrypts all household backups. |
| Household recovery `salt` (16 B) — **A** | NVS `HH_RECOVERY_SALT` | KDF salt | household-wide, public | Yes (backup header) | Yes | No | No | Salt is not secret; carried in the header. |
