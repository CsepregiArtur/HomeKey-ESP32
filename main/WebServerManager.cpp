 ...
 *
 * @details Stops the HTTP server if it is running, and stops and deletes the periodic
 * status timer.
 */
...
void WebServerManager::setupRoutes() {
  ...
  RouteConfig routes[] = {
      ...
      // No OTA routes. The single-slot layout has one application slot and no OTA data
      // partition, so a firmware upload has nowhere to be written and nothing to switch
      // to afterwards. See docs/content/SINGLE_SLOT_LAYOUT.md.
...
// ============================================================================
// This file no longer contains an over-the-air update path.
//
// It used to carry two of them: a GitHub release updater that downloaded and flashed a
// firmware image plus the Web UI filesystem image, and a browser upload endpoint for the
// same two things. Both wrote an application image into a second application slot and
// then switched the boot partition to it.
//
// The single-slot layout has one `factory` slot and no OTA data partition, so there is
// nowhere to write an image and nothing to switch to afterwards. Firmware is installed
// over serial, and the Web UI image travels in the same flash. See
// docs/content/SINGLE_SLOT_LAYOUT.md.
// ============================================================================
